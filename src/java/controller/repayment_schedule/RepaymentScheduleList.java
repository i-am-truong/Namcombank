...
public class RepaymentScheduleList extends HttpServlet {
    ...
    private String processPayment(int scheduleId, Customer customer) throws Exception {
        ...
        // Process the payment
        // 1. Update customer balance by subtracting the amount due
        BigDecimal newBalance = customerBalance.subtract(amountDue);
        boolean balanceUpdated = customerDAO.updateBalance(customer.getCustomerId(), newBalance);
        
        if (!balanceUpdated) {
            throw new Exception("Failed to update customer balance.");
        }
        
        // 2. Update repayment schedule status
        boolean statusUpdated = repaymentScheduleDAO.updateStatus(scheduleId, "PAID");
        
        if (!statusUpdated) {
            // If status update fails, revert the balance change
            customerDAO.updateBalance(customer.getCustomerId(), customerBalance);
            throw new Exception("Failed to update payment status. Your balance has not been charged.");
        }
